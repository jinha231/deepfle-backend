...
async function _rdSyncConvCols() {
  ...
  for (const c of conversions) {
    ...
    if (existing) {
      ...
      cols.push({key, label: c.solution_metric, type: 'metric', vis: false, fmt: 'N', group: c.col_group || 'conv', dynamic: true});
    }
  }
...
async function _ruSavePasteData() {
  ...
  const outRows = rows.map(r => {
    ...
    headers.forEach((h,i) => {
      ...
      const raw = (r[i]||'').trim();
      if (h.metric_key === 'date') { row.date = raw; return; }
  ...
  if (!outRows.length) { showToast('유효한 날짜가 있는 행이 없습니다', 'warning'); return; }
