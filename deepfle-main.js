...
async function _renderDashBody() {
  ...
  // 수기 매체와 동일한 이름/키로 실 API 조회(주로 수기 전환 데이터의 conv)에 섞여 들어온 항목은
  // 별도 행으로 중복 표시하지 않고 수기 매체 시리즈에 흡수한다.
...
async function renderMediaReportResult() {
  ...
  // API 실데이터 로드 — 특정 수기 매체를 선택해도 수기 전환 데이터(manual_conv_data)가 이 경로로 섞여 들어올 수 있으므로
  // 조회 자체는 그대로 시도하고, 아래에서 수기 매체 시리즈에 흡수 처리한다.
  let series;
