...
async function _ruRenderPasteSection() {
  ...
  const allMedia = [...MEDIA_DATA.filter(m=>m.on).map(m=>m.name), ...MANUAL_MEDIA.map(m=>m.name)];
  if (!allMedia.length) {
    el.innerHTML = `<div class="card" style="padding:40px;text-align:center;color:var(--gray-400);"><div style="font-size:32px;margin-bottom:12px;">📋</div><div>운영중인 매체가 없습니다.</div><div style="font-size:12px;margin-top:6px;">설정 &gt; 매체 연동에서 매체를 먼저 켜세요.</div></div>`;
    return;
  }
  ...
  el.innerHTML = `
    <div class="card" style="margin-bottom:16px;">
      <div class="card-header" style="flex-wrap:wrap;gap:8px;">
        <div><div class="card-title">수동 업로드</div><div class="card-sub">매체 리포트에서 헤더를 포함해 그대로 복사한 원본을 붙여넣으면, 설정 &gt; 리포트 설정 &gt; 컬럼 매핑에 따라 자동 분류됩니다</div></div>
      </div>
