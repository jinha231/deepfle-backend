...
async function _fetchManualConvData() {
  if (!currentAccount) return [];
  try {
    const res = await DEEPFLE_API.get(`/accounts/${currentAccount.id}/manual-conv-data`);
    return res.rows || [];
  } catch(e) { return []; }
}
  ...
  try {
    ...
    const rows = conversions.map(c=>{
      ...
      const connBtn = c.source === 'manual' ? '' :
        `<button class="btn btn-xs" style="${_connBtnStyle(hasConfig, isLive)}" onclick="openConvConnModal(${c.id},'${c.source}')">🔑 API 키${hasConfig?' 수정':' 설정'}</button>`;
      return `
      <tr data-id="${c.id}" style="border-left:3px solid ${hasConfig ? (isLive ? '#22C55E' : '#F59E0B') : 'var(--primary)'};background:linear-gradient(90deg,rgba(37,99,235,0.05) 0%,transparent 60%);">
        ...
        <td style="text-align:center;">
          <div style="display:flex;flex-direction:column;gap:4px;align-items:center;" class="conv-conn-btn">
            ${editable?`<button class="btn btn-xs btn-outline" onclick="saveConversion(${c.id},this)">저장</button>`:''}
            <div class="conv-conn-badge">${connBadge}</div>
            ${editable?connBtn:''}
            ${editable?`<button class="btn btn-xs btn-danger-outline" onclick="deleteConversion(${c.id})">삭제</button>`:'-'}
          </div>
        </td>
      </tr>`;
    }).join('');
    el.innerHTML = `
      <table class="data-table" style="width:100%;font-size:12px;">
        <thead><tr><th style="width:18%;">출처</th><th style="width:24%;">출처 지표명</th><th style="width:24%;">솔루션 지표명</th><th style="width:12%;">유형</th><th style="width:22%;">관리</th></tr></thead>
...
// 출처 드롭다운 변경 시 저장 전에도 연동 배지/버튼/행 테두리를 즉시 반영
function _updateConvRowConnUI(selectEl, convId) {
  ...
  const btnWrap = tr.querySelector('.conv-conn-btn');
  const existingBtn = btnWrap?.querySelector('button[onclick^="openConvConnModal"]');
  if (existingBtn) existingBtn.remove();
  if (btnWrap && source !== 'manual') {
    const btn = document.createElement('button');
    btn.className = 'btn btn-xs';
    btn.style.cssText = _connBtnStyle(hasConfig, isLive);
    btn.textContent = `🔑 API 키${hasConfig?' 수정':' 설정'}`;
    btn.setAttribute('onclick', `openConvConnModal(${convId},'${source}')`);
    const badgeDiv = btnWrap.querySelector('.conv-conn-badge');
    if (badgeDiv) badgeDiv.insertAdjacentElement('afterend', btn);
    else { const saveBtn = btnWrap.querySelector('button'); if (saveBtn) saveBtn.insertAdjacentElement('afterend', btn); else btnWrap.prepend(btn); }
  }
}
