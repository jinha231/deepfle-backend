...
async function _renderDashBody() {
  ...
  let series;
  let _dashConvSourceSum = 0;  // GA4 등 전환 소스는 매체 시리즈에서 빼고 총계에만 반영
  ...
  // 수기 매체: 백엔드(/metric-data)가 이미 manual_metrics를 병합해서 돌려주므로,
  // 거기 존재하는 매체는 key/label만 수기 매체 표기(id, "(수기)")로 통일해서 그대로 쓰고,
  // 백엔드에 아직 없는(로컬에만 남아있는, 미동기화) 매체만 로컬 폴백으로 보충한다 — 이중 합산 방지.
  const _seriesMediaNames = new Set(series.map(s => s.key));
  series.forEach(s => {
    const mm = MANUAL_MEDIA.find(m => m.name === s.key);
    if (mm) { s.key = mm.id; s.label = mm.name + ' (수기)'; s.color = mm.color || s.color; }
  });
  const _manualSeries = _getManualMediaSeries(dates).filter(ms => {
    const mm = MANUAL_MEDIA.find(m => m.id === ms.key);
    return !(mm && _seriesMediaNames.has(mm.name));
  });
  ...
  // 수기 전환 데이터는 백엔드(/metric-data)에서 이미 합산되어 totalConv에 포함됨. GA4 등 전환소스는 매체 시리즈에서 빠졌으므로 별도 가산
  ...
  // Media table rows (ON/OFF 제외) — 값은 MEDIA_DATA 스냅샷이 아니라 선택된 기간의 series(실 연동+수동 업로드 병합)에서 가져온다
  const _ovPixelData = _getMediaPixels();
  const _dashOnTotals = {cost:0, imp:0, click:0, conv:0, rev:0}; // 합계 행(ON 매체만)용 누적
  const mediaTableRows = MEDIA_DATA.map((m)=>{
    const sMatches = series.filter(sv => sv.key === m.key || sv.key === m.name);
    const mCost = sum(sMatches.flatMap(sv=>sv.cost));
    const mImp  = sum(sMatches.flatMap(sv=>sv.imp));
    ...
    if (m.on) {
      _dashOnTotals.cost += displaySpend; _dashOnTotals.imp += mImp; _dashOnTotals.click += mClick;
      _dashOnTotals.conv += mConv; _dashOnTotals.rev += mRev;
    }
    ...
    return `<tr>
      ...
      <td class="text-right num">₩${displaySpend.toLocaleString()}</td>
    ...
    return `<tr>
      <td><div class="media-logo"><div class="media-dot" style="background:${m.color||'#64748B'}"></div>${m.name} <span style="font-size:9px;color:var(--gray-400);">(수기)</span></div></td>
      ${USE_DAILY_BUDGET ? `<td class="text-right num">-</td>` : ''}
      <td class="text-right num">₩${mCost.toLocaleString()}</td>
      <td class="text-right num">${fmtN(mImp)}</td>
      <td class="text-right num">${fmtN(mClick)}</td>
      <td class="text-right">${ctrVal}</td>
      <td class="text-right num">${fmtN(mConv)}</td>
      <td class="text-right" style="color:${roasVal>=400?'var(--success)':roasVal<300?'var(--danger)':''};">${roasVal}%</td>
      <td class="text-right num">₩${cpaVal.toLocaleString()}</td>
    </tr>`;
  }).join('');

  // 디바이스별 / 상품유형별 breakdown (실 API 데이터 — 없으면 정직하게 빈 상태 표시)
  ...
  const DEVICE_LABELS_D = {mobile:'모바일',desktop:'PC/데스크탑',tablet:'태블릿',ctv:'CTV'};
  ...
  const CAMPAIGN_TYPE_LABELS_D = {SEARCH:'검색광고',DISPLAY:'디스플레이',PERFORMANCE_MAX:'PMax',SHOPPING:'쇼핑',VIDEO:'동영상',BIZBOARD:'비즈보드',CHANNEL_MSG:'채널메시지',CONVERSION:'전환',AWARENESS:'인지도',TRAFFIC:'트래픽',WEB_SITE:'파워링크',BRAND:'브랜드검색'};
  ...
  // 전체 매체 동기화 버튼 (구 공유 링크 자리로 이동)
  const syncBtn = (editable && DEEPFLE_API.live)
    ? `<button class="btn btn-sm btn-primary" onclick="syncAllConnectors()">⟳ 전체 매체 동기화</button>`
    : '';
  ...
  body.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
      <div class="card mr-section">
        ...
        ${_devEntries.length ? `${_devBar}<div style="overflow-x:auto;margin-top:8px;"><table class="data-table" style="width:100%;font-size:12px;"><thead><tr><th>디바이스</th><th class="text-right">광고비</th><th class="text-right">비중</th>${_devCompHeader}</tr></thead><tbody>${_devRows}</tbody></table></div>` : _noDataHtml}
      </div>
      <div class="card mr-section">
        <div class="card-header"><div><div class="card-title">상품유형별 성과</div><div class="card-sub">캠페인 유형별 광고비 비중</div></div></div>
        ${_prodEntries.length ? `${_prodBar}<div style="overflow-x:auto;margin-top:8px;"><table class="data-table" style="width:100%;font-size:12px;"><thead><tr><th>상품유형</th><th class="text-right">광고비</th><th class="text-right">비중</th>${_prodCompHeader}</tr></thead><tbody>${_prodRows}</tbody></table></div>` : _noDataHtml}
      </div>
    </div>
      ...
      <div style="overflow-x:auto;">
        <table>
          ...
          <tbody>${mediaTableRows}${(()=>{
            ...
            const tSpend=_dashOnTotals.cost + sum(manualSeries.map(s=>sum(s.cost)));
            const tImp=_dashOnTotals.imp + sum(manualSeries.map(s=>sum(s.imp)));
            const tClick=_dashOnTotals.click + sum(manualSeries.map(s=>sum(s.click)));
            const tConv=_dashOnTotals.conv + sum(manualSeries.map(s=>sum(s.conv)));
            const tRev=_dashOnTotals.rev + sum(manualSeries.map(s=>sum(s.revenue)));
            const tCtr=tImp?(tClick/tImp*100).toFixed(2)+'%':'-';
            const tRoas=tSpend?(tRev/tSpend*100).toFixed(0)+'%':'-';
    ...
    ${(()=>{
      ...
      const lines = ['효율변화','인사이트','제안'].map(label =>
        `<b>[${label}]</b> 실제 데이터 확인 후 AI가 생성합니다.`
      );
      ...
      return `<div class="card mr-section">
        <div class="card-header">
          <div><div class="card-title">인사이트</div><div class="card-sub">${_isComp?`비교기간(${compFrom}${compTo?' ~ '+compTo:''}) 대비 매체별 변화`:'전일 대비 성과 분석'} · AI 자동 생성 준비 중</div></div>
...
function _wizBodyStep3() {
  ...
  return `<div>
    ...
    <div style="margin-top:10px;font-size:11px;color:var(--gray-400);">⏭ 건너뛰기 가능 — 이후 전환데이터 입력 › 전환설정에서 추가 가능</div>
  </div>`;
}
...
// ============================================================
// SHARE VIEW — 리포트 이메일 발송에 포함되는 공개 읽기 전용 링크
// ============================================================
...
function crSendNow(idx) {
  ...
  // 데모 모드에서는 브라우저에 저장된 실제 EmailJS 설정이 남아있더라도 절대 실발송하지 않는다.
  const ejReady = !_demoMode && cfg.publicKey && cfg.serviceId && cfg.templateReport && typeof emailjs !== 'undefined';
...
async function openReportViewer(idx) {
  ...
  if (useReal) {
    // 매체별 집계 (GA4 등 전환 소스는 매체가 아니므로 제외하고 총계에만 반영)
    const mediaAgg = {};
    apiRows.forEach(r => {
...
function _timeAgo(dateStr) {
  if (!dateStr) return '';
  // SQLite datetime('now')는 타임존 표기 없는 UTC 문자열이므로 'Z'를 붙여 UTC로 명시 파싱
...
// audit_log의 action 코드 → 화면 표시용 한글 라벨
const ACTION_LABELS = {
  connector_sync:'매체 연동 동기화', account_sync_connectors:'전체 매체 동기화',
  account_create:'광고주 계정 생성', account_update:'계정 정보 수정',
  invite_advertiser:'광고주 초대', approve_user:'사용자 승인', delete_user:'사용자 삭제',
  ad_account_connect:'광고계정 연결', ad_account_update:'광고계정 수정',
  media_toggle:'매체 ON/OFF 변경', media_budget:'일일 소진한도 설정',
  rule_create:'자동화 규칙 생성', rule_toggle:'자동화 규칙 ON/OFF', rule_delete:'자동화 규칙 삭제',
  rule_execute:'자동화 규칙 실행', rule_undo:'자동화 실행 취소',
  conversion_create:'전환설정 추가', conversion_delete:'전환설정 삭제',
  role_change:'권한 변경', metric_pull:'데이터 갱신',
  save_credentials:'API 자격증명 저장', delete_credentials:'API 자격증명 삭제',
  save_media_metric_map:'컬럼 매핑 저장',
  upload_manual_metrics:'수동 데이터 업로드',
  save_manual_conv_data:'수기 전환 데이터 입력', delete_manual_conv_data:'수기 전환 데이터 삭제',
  report_config_save:'리포트 설정 저장', report_export:'리포트 내보내기',
  media_sync:'매체 동기화',
};

async function renderWorkspace() {
  const r = currentUser.role;
  const canSeeTeamMsg = CAN_EDIT(r); // 마스터·사용자만

  // 활동 피드: audit_log 기반 실데이터 (=알림 내용)
  const feedEl = document.getElementById('activityFeed');
  if (feedEl && currentAccount) {
    feedEl.innerHTML = `<div style="padding:20px;text-align:center;color:var(--gray-400);font-size:13px;">불러오는 중…</div>`;
    try {
      ...
      feedEl.innerHTML = entries.length ? entries.map(a => {
        ...
        const userName = _escapeHtml(a.user_name || '알 수 없음');
        const detail = a.detail ? ' — ' + _escapeHtml(a.detail) : '';
      ...
      }).join('') : `<div style="padding:20px;text-align:center;color:var(--gray-400);font-size:13px;">아직 활동 내역이 없습니다.</div>`;
    } catch(e) {
      feedEl.innerHTML = `<div style="padding:20px;text-align:center;color:var(--gray-400);font-size:13px;">활동 내역을 불러오지 못했습니다.</div>`;
    }
  }

  // 팀 메시지: 같은 계정을 관리하는 마스터·사용자만
...
async function _renderTeamMessages() {
  ...
  listEl.innerHTML = `<div style="padding:16px;text-align:center;color:var(--gray-400);font-size:12px;">불러오는 중…</div>`;
  try {
    const res = await DEEPFLE_API.get(`/accounts/${currentAccount.id}/messages`);
    const msgs = (res.messages || []).slice().reverse(); // 오래된순으로 표시
    ...
    }).join('') : `<div style="padding:16px;text-align:center;color:var(--gray-400);font-size:12px;">아직 메시지가 없습니다. 첫 메시지를 남겨보세요.</div>`;
    listEl.scrollTop = listEl.scrollHeight;
  } catch(e) {
    listEl.innerHTML = `<div style="padding:16px;text-align:center;color:var(--gray-400);font-size:12px;">메시지를 불러오지 못했습니다.</div>`;
  }
  if (composerEl) {
    composerEl.innerHTML = `
      <div class="comment-input-row">
        <div class="mention-dropdown" id="teamMsgMentionDrop"></div>
        <input class="comment-input" id="teamMsgInput" placeholder="메시지 작성… (@로 멘션)"
               oninput="_onTeamMsgInput(this)" onkeydown="if(event.key==='Enter')sendTeamMessage()">
        <span class="comment-send" onclick="sendTeamMessage()">전송</span>
      </div>`;
  }
}

// ── 팀 메시지 입력: @멘션 드롭다운 (마스터·사용자만 검색 대상) ──
...
async function renderMediaReportResult() {
  ...
  // API 실데이터 로드 — 특정 수기 매체를 선택해도 /metric-data가 manual_metrics·manual_conv_data를
  // 함께 병합해서 돌려주므로 조회 자체는 그대로 시도한다 (아래에서 수기 매체 표기로 정리).
  let series;
  let _mrConvSourceSum = 0;  // GA4 등 전환 소스는 매체 시리즈에서 빼고 총계에만 반영
  if (!DEEPFLE_API.USE_MOCK) {
    try {
  ...
  // 거기 존재하는 매체는 key/label만 수기 매체 표기(id, "(수기)")로 통일해서 그대로 쓰고,
  // 백엔드에 아직 없는(로컬에만 남아있는, 미동기화) 매체만 로컬 폴백으로 보충한다 — 이중 합산 방지.
  const _seriesMediaNamesMr = new Set(series.map(s => s.key));
  series.forEach(s => {
    const mm = MANUAL_MEDIA.find(m => m.name === s.key);
    if (mm) { s.key = mm.id; s.label = mm.name + ' (수기)'; s.color = mm.color || s.color; }
  });
  const manualFilter = (media && media !== '__all__') ? media : null;
  const manualSeriesMr = _getManualMediaSeries(dates)
    .filter(s => !manualFilter || s.key === manualFilter || s.label.startsWith(manualFilter))
    .filter(ms => {
      const mm = MANUAL_MEDIA.find(m => m.id === ms.key);
      return !(mm && _seriesMediaNamesMr.has(mm.name));
    });
...
// ============================================================
// 서버(DB) 저장 — Raw 다운로드·대시보드·리포트 요약에 자동 반영됨 (Volume에 영구 보존)
const MANUAL_CONV_KEY = 'deepfle_manual_conv_data'; // 구버전(로컬 저장) 마이그레이션 전용
...
async function _migrateLegacyManualConvData() {
  ...
  try {
    ...
    showToast('브라우저에 남아있던 수기 전환 데이터를 서버로 이전했습니다', 'success');
  } catch(e) {}
}
...
async function renderManualConvData() {
  ...
  // 기존 저장값 맵: key = "date__media__conv_id"
  ...
  <div class="card" style="margin-bottom:16px;">
    <div class="card-header">
      <div><div class="card-title">수기 전환 데이터 입력</div>
        <div class="card-sub">기간 × 매체 × 전환지표 전체를 한 표에서 입력하거나 엑셀에서 복사·붙여넣기 합니다</div>
      </div>
    </div>
    ...
    <div style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px;">
      ...
      <button class="btn btn-sm btn-outline" title="표에 입력된 모든 값을 지웁니다 (저장 전까지는 반영되지 않습니다)" onclick="_mcvClearForm()">초기화</button>
    </div>
    ...
    <div style="background:#F0F9FF;border:1px solid #BAE6FD;border-radius:6px;padding:8px 12px;font-size:11px;color:#0369A1;margin-bottom:12px;display:flex;align-items:center;gap:8px;">
      <span>💡</span>
      <span>엑셀에서 <b>날짜 순 × (매체 · 전환지표) 순</b>으로 값만 복사(Ctrl+C)한 후, 원하는 시작 셀을 클릭하고 붙여넣기(Ctrl+V) 하세요. 매체 하나에 지표 ${convs.length}개가 옆으로 나란히 있어 광고주 원본 표를 그대로 붙여넣을 수 있습니다. Tab 구분·줄 바꿈 자동 인식.</span>
    </div>
    ...
    <div style="overflow-x:auto;">
      <table class="data-table" id="mcvMatrix" style="font-size:12px;min-width:500px;">
        <thead>
          <tr>
            <th rowspan="2" style="min-width:90px;background:var(--gray-50);vertical-align:middle;">날짜</th>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;">
      <span style="font-size:11px;color:var(--gray-400);">${dateList.length}일 × ${allMedia.length}개 매체 × ${convs.length}개 지표</span>
  ...
  ${(()=>{
    ...
    return `<div class="card"><div class="card-header">
      <div><div class="card-title">입력 이력</div><div class="card-sub">날짜 클릭 시 세부 내용 확인·삭제</div></div>
      <span style="font-size:11px;color:#D97706;background:#FFFBEB;border:1px solid #FDE68A;padding:3px 10px;border-radius:12px;white-space:nowrap;">⚠ 저장 후 1주일이 지나면 자동 삭제됩니다</span>
    </div>${histBody}</div>`;
  })()}`;
}

// 저장 여부와 무관하게 표에 입력된 값만 전부 비운다 (서버 데이터에는 영향 없음).
...
async function saveManualConvData() {
  ...
  if (!entries.length) { showToast('입력된 값이 없습니다','warning'); return; }
  try {
    await DEEPFLE_API.post(`/accounts/${currentAccount.id}/manual-conv-data`, {entries});
    showToast(`${entries.length}건 저장 완료 — Raw 다운로드·대시보드·리포트에 자동 반영됩니다`, 'success');
...
const EXT_CONNECT_FIELDS = {
  ga4: {
    guide: '💡 Google Cloud Console → IAM 및 관리자 → 서비스 계정 생성 → JSON 키 발급 후, 그 서비스계정 이메일을 GA4 관리 → 속성 액세스 관리에 "뷰어" 권한으로 추가하세요.',
    fields: [
      { key:'propertyId',         label:'GA4 속성 ID',        placeholder:'예: 123456789', required:true, hint:'GA4 관리 > 속성 설정', type:'text' },
      { key:'serviceAccountJson', label:'서비스 계정 JSON 키', placeholder:'{"type":"service_account","client_email":"...","private_key":"...", ...}', required:true, hint:'Google Cloud Console에서 발급받은 JSON 키 파일 내용 전체를 붙여넣으세요', type:'textarea' },
    ]
  },
  acecounter: {
...
// ============================================================
// 설정 > 리포트 설정 > 컬럼 매핑 (매체 리포트 원본 컬럼명 ↔ 솔루션 표준 컬럼명)
// Raw 업로드 "수동 업로드"에서 붙여넣은 헤더를 자동 인식하는 데 사용된다.
// ============================================================
...
async function renderSettingMediaMapping() {
  ...
  if (!allMedia.length) {
    el.innerHTML = `<div class="card"><div style="padding:24px;text-align:center;color:var(--gray-400);font-size:13px;">운영중인 매체가 없습니다. <b>매체 연동</b>에서 매체를 먼저 켜세요.</div></div>`;
    return;
  }
  ...
  const accRows = allMedia.map(m => {
    ...
    const tableRows = STANDARD_FIELDS.map(f => {
      ...
      return `<tr>
        ...
        <td><input class="form-input map-f" data-key="${f.key}" data-field="provider_field" value="${provVal}" placeholder="이 매체 리포트의 원본 컬럼명" ${editable?'':'disabled'} style="height:30px;font-size:12px;"></td>
      </tr>`;
    }).join('');
    return `
    <div class="conn-acc">
      <div class="conn-acc-head ${isOpen?'open':''}" onclick="toggleMapAccordion('${mNameEsc}')">
        ...
        <span class="badge ${mappedCnt?'badge-green':'badge-gray'}" style="font-size:10px;white-space:nowrap;">${mappedCnt}/${STANDARD_FIELDS.length} 매핑됨</span>
        <span class="conn-acc-chev">▼</span>
      </div>
      ${isOpen ? `
      <div class="conn-acc-body">
        <div style="overflow-x:auto;">
          <table class="data-table" style="width:100%;font-size:12px;min-width:480px;">
            <thead><tr><th style="width:30%;">표준 필드</th><th style="width:70%;">이 매체의 원본 컬럼명</th></tr></thead>
            <tbody>${tableRows}</tbody>
          </table>
        </div>
        ${editable ? `<div style="margin-top:10px;text-align:right;"><button class="btn btn-sm btn-primary" onclick="saveMediaMetricMap('${mNameEsc}',this)">저장</button></div>` : ''}
      </div>` : ''}
    </div>`;
  }).join('');
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-sub">매체를 펼쳐서 리포트 원본 컬럼명 → 솔루션 표시명을 매핑하세요. Raw 업로드의 "수동 업로드"가 이 매핑으로 붙여넣은 데이터를 자동 인식합니다.</div>
        </div>
      </div>
      ${accRows}
    </div>`;
...
async function saveMediaMetricMap(mediaName, btn) {
  ...
  try {
    ...
    showToast('컬럼 매핑이 저장되었습니다', 'success');
    renderSettingMediaMapping();
  } catch(e) {
    showToast(`저장 실패: ${e.message}`, 'error');
  }
...
const LIVE_CONV_SOURCES = ['ga4'];  // 실제 API 연동이 구현된 소스 (나머지는 자격증명 저장만 가능)
// metric_data에는 GA4 등 전환 소스가 media='ga4' 형태로 섞여 들어옴 — 매체별 표/시리즈에서는 제외하고
// 총 전환수 계산에만 반영하기 위한 헬퍼
...
// 전환설정 행의 연동 상태 배지/버튼 스타일 — 저장 전(미설정)·저장됨(준비중)·실연동(연동됨) 3단계를 한눈에 구분
function _connStatusBadgeHtml(hasConfig, isLive) {
  if (!hasConfig) return `<span class="badge badge-gray" style="font-size:9px;white-space:nowrap;">⚪ 키 미설정</span>`;
  if (isLive) return `<span class="badge badge-green" style="font-size:9px;white-space:nowrap;">🟢 연동됨</span>`;
  return `<span class="badge badge-orange" style="font-size:9px;white-space:nowrap;" title="자격증명은 저장되어 있으나, 실제 API 연동(데이터 수집)은 아직 준비 중입니다.">🔵 키 저장됨 · 준비중</span>`;
}
...
async function renderConversionSettings() {
  ...
  try {
    const { conversions } = await DEEPFLE_API.get(`/accounts/${currentAccount.id}/conversion-settings`);
    window._connSourcesCache = conversions;  // 오디언스 타겟팅 등에서 연동 상태 판정용으로 재사용
    ...
    const rows = conversions.map(c=>{
      ...
      const connBtn = c.source === 'manual' ? '' :
        `<button class="btn btn-xs" style="${_connBtnStyle(hasConfig, isLive)}width:100%;" onclick="openConvConnModal(${c.id},'${c.source}')">🔑 키${hasConfig?' 수정':' 설정'}</button>`;
      return `
      <tr data-id="${c.id}" style="border-left:3px solid ${hasConfig ? (isLive ? '#22C55E' : '#F59E0B') : 'var(--primary)'};background:linear-gradient(90deg,rgba(37,99,235,0.05) 0%,transparent 60%);">
        ...
        <td>
          <div style="display:flex;align-items:center;gap:6px;">
            ...
            <select class="cv-f" data-k="col_group" ${dis} title="Raw 업로드 컬럼 설정에서 표시될 태그" style="white-space:nowrap;font-size:10px;font-weight:600;background:var(--primary-light);color:var(--primary);padding:2px 4px;border-radius:10px;border:none;">
              <option value="conv" ${c.col_group!=='metric'?'selected':''}>전환</option>
              <option value="metric" ${c.col_group==='metric'?'selected':''}>지표</option>
            </select>
          </div>
        </td>
        ...
        <td style="text-align:center;vertical-align:middle;">
          ...
          <div style="display:flex;gap:4px;justify-content:center;" class="conv-conn-btn">
            <button class="btn btn-xs btn-outline" style="flex:1;" onclick="saveConversion(${c.id},this)">저장</button>
            <button class="btn btn-xs btn-danger-outline" style="flex:1;" onclick="deleteConversion(${c.id})">삭제</button>
          </div>
    ...
    el.innerHTML = `
      <table class="data-table" style="width:100%;font-size:12px;table-layout:fixed;">
        <thead><tr><th style="width:17%;">출처</th><th style="width:22%;">출처 지표명</th><th style="width:23%;">솔루션 지표명</th><th style="width:12%;">유형</th><th style="width:26%;">관리</th></tr></thead>
...
// 출처 드롭다운 변경 시 즉시 배지/버튼/행 테두리를 반영하고, 서버에도 바로 저장한다.
// (드롭다운만 바꾸고 "저장"을 안 누른 상태로 다른 행을 조작하면 전체 재조회 시 이 변경이 사라지는 문제 방지)
async function _updateConvRowConnUI(selectEl, convId) {
  ...
  if (keyBtnEl) {
    keyBtnEl.innerHTML = source === 'manual' ? '' :
      `<button class="btn btn-xs" style="${_connBtnStyle(hasConfig, isLive)}width:100%;" onclick="openConvConnModal(${convId},'${source}')">🔑 키${hasConfig?' 수정':' 설정'}</button>`;
  }

  // 서버에도 즉시 반영 — 다른 행 조작으로 인한 전체 재조회에도 선택값이 유지되도록
  if (row && row.source !== source) {
    try {
      await DEEPFLE_API.patch(`/conversion-settings/${convId}`, {source});
      row.source = source;  // 로컬 캐시도 즉시 동기화
    } catch(e) { /* 실패해도 조용히 — 사용자가 "저장" 버튼으로 다시 시도 가능 */ }
  }
}
...
// ── 전환설정 > 소스 연동 자격증명 모달 (GA4/MMP 등) ──
let _convConnTarget = null;
let _convConnSource = null;  // 드롭다운에서 아직 저장 전인 source 값 — 캐시(window._connSourcesCache)가 아닌 이 값을 신뢰

function openConvConnModal(convId, source) {
  const schema = EXT_CONNECT_FIELDS[source];
  const sol = EXT_SOLUTIONS.find(s => s.id === source);
  if (!schema || !sol) { showToast('이 출처는 연동 설정이 필요하지 않습니다', 'info'); return; }
  _convConnTarget = convId;
  _convConnSource = source;
  const row = (window._connSourcesCache || []).find(c => c.id === convId);
  // source가 아직 저장 전(드롭다운만 변경)이면 이전 소스의 config는 무의미하므로 비움
  let savedConfig = {};
  if (row?.source === source) {
    try { savedConfig = JSON.parse(row?.config || '{}'); } catch(e) {}
  }
  document.getElementById('convConnModalTitle').textContent = sol.name + ' 연동';
  document.getElementById('convConnModalSub').textContent = sol.category + ' · ' + sol.desc;
  ...
  document.getElementById('convConnFields').innerHTML = schema.fields.map(f => {
    ...
    const sensitivePh = ((isPw || isTa) && savedVal ? '(저장된 값 있음 — 변경 시에만 입력)' : (f.placeholder || '')).replace(/"/g,'&quot;');
...
async function submitConvConnect() {
  ...
  try {
    // 자격증명 저장과 동시에 source 변경(드롭다운에서 선택했지만 아직 "저장" 안 누른 경우)도 함께 확정
    ...
    const msg = LIVE_CONV_SOURCES.includes(source)
      ? 'API 키가 저장되었습니다 — 리포트 설정에서 "지금 갱신"을 실행하면 실제 데이터를 가져옵니다'
      : 'API 키가 저장되었습니다 (실제 데이터 수집은 API 연동 구현 후 적용됩니다)';
...
// 전환설정(활성)의 개별 지표를 Raw 업로드 컬럼 목록에 동적 컬럼으로 반영.
// 이미 추가된 컬럼은 순서·표시여부는 유지한 채 라벨/그룹만 최신화하고, 새 지표는 목록 끝에 추가한다.
// 삭제되었거나 비활성화된 지표의 컬럼(dynamic:true)은 목록에서 함께 제거한다.
async function _rdSyncConvCols() {
  ...
  for (const c of conversions) {
    ...
    if (existing) {
      ...
      existing.vis = true; // 전환데이터 입력값이 안 보인다는 혼선 방지 — 전환지표 컬럼은 항상 기본 표시
    } else {
...
async function fetchRawData() {
  ...
  // 전환설정(수기 전환 데이터 입력) 값 병합 — 지표별 conv_id_{id} 필드로 각 행에 합류
  if (currentAccount && DEEPFLE_API.live) {
    try {
      ...
      for (const cr of convRows) {
        ...
        target.conv = (target.conv || 0) + (cr.value || 0); // 전환(합계)에도 개별 전환지표 값을 반영
      }
    } catch(e) {}
  }
  _rdRows=rows;
...
function _rdRenderTable(breakdown) {
  ...
  // 합계행 — 기본 지표 + 화면에 보이는 동적 전환지표(conv_id_*) 컬럼 모두 합산
  const sumRow={};
...
function rdDownloadCSV(){
  ...
  // 계정명은 맨 앞, 업종·카테고리는 날짜 컬럼 바로 다음(=매체 컬럼 앞)에 끼워넣는다
  const dateIdx = visCol.findIndex(c=>c.key==='date');
  const insertAt = dateIdx>=0 ? dateIdx+1 : 0;
  const header=['계정명', ...visCol.slice(0,insertAt).map(c=>c.label), '업종','카테고리', ...visCol.slice(insertAt).map(c=>c.label)].join(',');
  const body=rows.map(r=>[accName, ...visCol.slice(0,insertAt).map(c=>_rdFmtCSV(c,r)), accInd, accCat, ...visCol.slice(insertAt).map(c=>_rdFmtCSV(c,r))].join(',')).join('\n');
  const csv='﻿'+header+'\n'+body;
  const blob=new Blob([csv],{type:'text/csv;charset=utf-8'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');
...
async function renderRawUpload() {
  ...
  el.innerHTML = `
    ...
    <div class="tab-pills" style="margin-bottom:16px;">
      ...
      <div class="tab-pill${_ruTab==='paste'?' active':''}" onclick="_ruSwitchTab('paste')">수동 업로드</div>
...
function _ruApplyDate() {
  ...
  // 같은 오퍼레이팅 그룹인 전환데이터 입력·Raw 다운로드도 같은 기간을 기본값으로 쓰도록 전파
  window._mcvFrom = _ruFrom; window._mcvTo = _ruTo;
  window._rdFrom  = _ruFrom; window._rdTo  = _ruTo;
  if (_ruTab === 'api') _ruLoadApiData();
  else if (_ruTab === 'paste') _ruRenderPasteSection();
  else _ruRenderManualSection();
}

function _ruSwitchTab(tab, manualScope) {
  _ruTab = tab;
  _ruPasteScopeManual = !!manualScope;
  renderRawUpload();
}

// ============================================================
// Raw 업로드 > 수동 업로드 (매체 리포트 원본을 그대로 붙여넣어 컬럼 매핑으로 자동 분류)
// ============================================================
let _ruPasteMedia = null;
let _ruPasteResult = null; // {headers:[{text, metric_key}], rows:[[...]]}
let _ruPasteScopeManual = false; // true면 "수기 매체 업로드" 탭에서 넘어온 붙여넣기 — 매체 드롭다운을 수기 매체로만 제한

async function _ruRenderPasteSection() {
  const el = document.getElementById('ruPasteBody');
  if (!el) return;
  const isManualScope = _ruPasteScopeManual;
  const allMedia = isManualScope
    ? MANUAL_MEDIA.map(m=>m.name)
    : [...MEDIA_DATA.filter(m=>m.on).map(m=>m.name), ...MANUAL_MEDIA.map(m=>m.name)];
  if (!allMedia.length) {
    el.innerHTML = isManualScope
      ? `<div class="card" style="padding:40px;text-align:center;color:var(--gray-400);"><div style="font-size:32px;margin-bottom:12px;">✏️</div><div>등록된 수기 매체가 없습니다.</div><div style="font-size:12px;margin-top:6px;">설정 &gt; 매체 연동에서 수기 매체를 먼저 추가하세요.</div></div>`
      : `<div class="card" style="padding:40px;text-align:center;color:var(--gray-400);"><div style="font-size:32px;margin-bottom:12px;">📋</div><div>운영중인 매체가 없습니다.</div><div style="font-size:12px;margin-top:6px;">설정 &gt; 매체 연동에서 매체를 먼저 켜세요.</div></div>`;
    return;
  }
  ...
  const backLink = isManualScope
    ? `<div style="font-size:11px;color:var(--primary-600, var(--primary));cursor:pointer;margin-bottom:10px;" onclick="_ruSwitchTab('manual')">← 수기 매체 업로드로 돌아가기</div>`
    : '';
  ...
  el.innerHTML = `
    ...
    <div class="card" style="margin-bottom:16px;">
      <div class="card-header" style="flex-wrap:wrap;gap:8px;">
        <div><div class="card-title">${isManualScope?'수기 매체 붙여넣기':'수동 업로드'}</div><div class="card-sub">매체 리포트에서 헤더를 포함해 그대로 복사한 원본을 붙여넣으면, 설정 &gt; 리포트 설정 &gt; 컬럼 매핑에 따라 자동 분류됩니다</div></div>
      </div>
      <div style="font-size:11px;color:var(--gray-400);margin-bottom:10px;">💡 디바이스 컬럼은 PC·데스크탑·Desktop / 모바일·Mobile·MO / 태블릿·테블릿·Tablet 등 다양한 표기를 자동으로 통일해서 저장합니다.</div>
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
        <label style="font-size:12px;font-weight:600;color:var(--gray-600);">매체</label>
        ...
        <span style="font-size:11px;color:var(--primary-600, var(--primary));cursor:pointer;" onclick="showPanel('settings');setTimeout(()=>{const t=document.querySelector('.tab-pill[onclick*=connection]');if(t)switchSettingTab(t,'mapping');},150);">컬럼 매핑 설정하기 →</span>
      </div>
      <textarea id="ruPasteArea" placeholder="매체 리포트에서 헤더 행을 포함해 복사(Ctrl+C)한 뒤 여기에 붙여넣으세요(Ctrl+V)" style="width:100%;min-height:120px;font-size:12px;font-family:monospace;padding:8px;border:1px solid var(--gray-200);border-radius:8px;resize:vertical;box-sizing:border-box;"></textarea>
      <div style="margin-top:10px;text-align:right;">
        <button class="btn btn-primary btn-sm" onclick="_ruClassifyPaste()">분류하기</button>
      </div>
    </div>
    <div id="ruPasteResult"></div>
    <div id="ruPasteHistory" style="margin-top:16px;">불러오는 중…</div>`;
  await _ruRenderPasteHistory();
}

// 최근 7주 이내 수동 업로드 이력 (audit_log 재사용 — 별도 저장소 없이 조회 시점 필터링만)
async function _ruRenderPasteHistory() {
  ...
  try {
    const res = await DEEPFLE_API.get(`/accounts/${currentAccount.id}/audit`);
    const cutoff = new Date(Date.now() - 49*86400000); // 7주
    entries = (res.audit || [])
      .filter(a => a.action === 'upload_manual_metrics' && new Date(a.created_at) >= cutoff)
      .slice(0, 30);
  } catch(e) { console.error('업로드 이력 로드 실패', e); el.innerHTML = ''; return; }
  if (!entries.length) {
    el.innerHTML = `<div class="card"><div class="card-header"><div class="card-title">업로드 이력</div><div class="card-sub">최근 7주 이내 수동 업로드 기록</div></div><div style="padding:20px;text-align:center;color:var(--gray-400);font-size:13px;">업로드 이력이 없습니다.</div></div>`;
    return;
  }
  ...
  el.innerHTML = `
    <div class="card">
      <div class="card-header"><div><div class="card-title">업로드 이력</div><div class="card-sub">최근 7주 이내 수동 업로드 기록</div></div></div>
...
async function _ruClassifyPaste() {
  ...
  if (lines.length < 2) { showToast('헤더 행과 데이터 행이 포함된 값을 붙여넣으세요', 'warning'); return; }
  if (!currentAccount) { showToast('계정을 선택하세요', 'warning'); return; }
...
function _ruRenderPasteResult() {
  ...
  const stdOptHtml = sel => `<option value="">미분류</option>` + STANDARD_FIELDS.map(f=>`<option value="${f.key}" ${f.key===sel?'selected':''}>${f.label}</option>`).join('');
  ...
  const notice = unmatchedCnt
    ? `<div style="font-size:11px;color:#D97706;background:#FFFBEB;padding:6px 10px;border-radius:6px;margin-bottom:8px;">⚠ 미분류 컬럼 ${unmatchedCnt}개 — 위 드롭다운에서 표준 필드를 지정하면 저장 시 반영되고, 다음부터 이 매체는 자동 인식됩니다.</div>`
    : `<div style="font-size:11px;color:#059669;background:#ECFDF5;padding:6px 10px;border-radius:6px;margin-bottom:8px;">모든 컬럼이 인식됐습니다.</div>`;

  el.innerHTML = `
    <div class="card">
      <div class="card-header" style="flex-wrap:wrap;gap:8px;">
        <div><div class="card-title">분류 결과</div><div class="card-sub">총 ${rows.length.toLocaleString()}행 · ${_ruPasteMedia}</div></div>
        <button class="btn btn-primary btn-sm" onclick="_ruSavePasteData()">💾 저장</button>
      </div>
      ${notice}
      ...
      ${rows.length>50?`<div style="padding:6px 4px;font-size:11px;color:var(--gray-400);">50행만 미리보기 표시 — 저장 시 전체 ${rows.length.toLocaleString()}행이 반영됩니다.</div>`:''}
    </div>`;
}
...
// 붙여넣은 원본 날짜 텍스트를 DB/조회가 기대하는 YYYY-MM-DD로 정규화한다.
// 매체 리포트마다 20260706(구분자 없음) · 2026.7.6 · 2026/7/6 등 형식이 제각각이라,
// 여기서 맞추지 않으면 문자열 비교인 날짜 범위 조회(Raw 다운로드 등)에서 그 행이 조용히 빠진다.
...
// 사용자가 다양한 표기(PC·데스크탑·Desktop / Mobile·모바일·MO / 테블릿·Tablet 등)로 입력한 디바이스명을
// 내부 표준 키(desktop/mobile/tablet/ctv)로 통일한다. 여기서 맞추지 않으면 Raw 다운로드·리포트의
// 디바이스별 집계가 같은 기기인데도 서로 다른 값으로 흩어져 표시된다.
// 알려진 표기와 매칭되지 않으면 원본 값을 그대로 둔다(임의로 지우지 않음).
const DEVICE_ALIASES = {
  desktop: ['pc', '데스크탑', '데스크톱', 'desktop'],
  mobile:  ['mobile', '모바일', 'mo', 'm'],
  tablet:  ['tablet', '태블릿', '테블릿', 'tab'],
  ctv:     ['ctv'],
};
function _normalizeDevice(raw) {
  const s = (raw || '').trim();
  if (!s) return '';
...
async function _ruSavePasteData() {
  ...
  if (dateIdx < 0) { showToast('"날짜"로 분류된 컬럼이 있어야 저장할 수 있습니다', 'warning'); return; }
  ...
  const outRows = rows.map(r => {
    ...
    headers.forEach((h,i) => {
      ...
      if (h.metric_key === 'device') { row.device = _normalizeDevice(raw); return; }
  ...
  if (!outRows.length) { showToast('유효한 날짜가 있는 행이 없습니다 (YYYYMMDD 또는 YYYY-MM-DD 형식인지 확인하세요)', 'warning'); return; }
  if (outRows.length < rows.length) {
    showToast(`날짜 형식을 인식하지 못한 ${rows.length - outRows.length}행은 제외하고 저장합니다`, 'warning');
  }

  // 이번에 지정된 매핑을 저장해서 다음부터는 이 매체를 자동 인식하도록 반영
  ...
  try {
    ...
    showToast(`${res.inserted}행 저장 완료 — Raw 다운로드에서 확인할 수 있습니다`, 'success');
    ...
    <div class="card" style="margin-bottom:16px;">
      <div class="card-header" style="flex-wrap:wrap;gap:8px;">
        <div>
          ...
          <div class="card-sub">등록된 수기 매체 ${MANUAL_MEDIA.length}개 · CSV 일괄 업로드 또는 매체별 붙여넣기</div>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <button type="button" class="btn btn-sm btn-outline" onclick="_ruPasteMedia=null;_ruSwitchTab('paste',true)">📋 붙여넣기로 업로드</button>
      ...
      <div style="background:#F0F9FF;border:1px solid #BAE6FD;border-radius:6px;padding:9px 13px;font-size:12px;color:#0369A1;margin-top:10px;line-height:1.6;">
        ...
        <span style="opacity:.8;">디바이스는 PC·데스크탑·Desktop / 모바일·Mobile·MO / 태블릿·테블릿·Tablet 등 다양한 표기를 자동으로 통일해서 저장합니다.</span><br>
...
async function renderRawDownload(){
  ...
  // 최신 매체 상태 반영
  if (DEEPFLE_API.live && currentAccount) await loadBackendMedia(currentAccount.id);
  // 날짜: 활성 프리셋 > Raw 업로드에서 전파된 기간 > 글로벌 날짜 > 기본값(최근 30일)
  const p = _rdActivePreset;
  const from = (p?.from) || window._rdFrom || window._globalFrom || new Date(Date.now()-30*86400000).toISOString().slice(0,10);
  const to   = (p?.to)   || window._rdTo   || window._globalTo   || new Date().toISOString().slice(0,10);
...
// 데모용 활동 피드 — 실제 audit_log 형태를 그대로 흉내내되 항상 "방금~며칠 전"처럼 보이도록 현재 시각 기준으로 계산
function _genDemoAuditRows() {
  const now = Date.now();
  const fmt = ms => new Date(now - ms).toISOString().slice(0,19).replace('T',' ');
  const u = {user_id:'u_master', user_name:'데모 관리자', role:'master'};
  return [
    {id:6, account_id:'demo_acc1', ...u, action:'account_sync_connectors', detail:'5개 매체 동기화 → 32개 캠페인', created_at: fmt(12*60000)},
    {id:5, account_id:'demo_acc1', ...u, action:'save_manual_conv_data',   detail:'수기 전환 데이터 3건 저장',       created_at: fmt(95*60000)},
    {id:4, account_id:'demo_acc1', ...u, action:'save_media_metric_map',   detail:'네이버 검색광고 컬럼 매핑 6건 저장', created_at: fmt(3*3600000)},
    {id:3, account_id:'demo_acc1', ...u, action:'conversion_create',      detail:'전환설정 추가: 구매 (ga4)',       created_at: fmt(26*3600000)},
    {id:2, account_id:'demo_acc1', ...u, action:'save_credentials',       detail:'구글 Ads API 자격증명 저장',       created_at: fmt(50*3600000)},
    {id:1, account_id:'demo_acc1', ...u, action:'account_create',         detail:'광고주 계정 생성: 데모 브랜드',     created_at: fmt(72*3600000)},
  ];
}
...
// 데모 모드 POST 응답 — 실제 백엔드를 절대 호출하지 않고, 클릭 시 UI가 자연스럽게 보이도록 그럴듯한 값만 흉내낸다.
function _buildDemoPostResponse(path, body) {
  if (/\/sync-connectors/.test(path)) {
    return {ok:true, jobId:1, mediaCount:_DEMO_MEDIA_BASE.length, totalCampaigns:_DEMO_MEDIA_BASE.length*6, usedFixture:true, perMedia:[]};
  }
  if (/\/messages$/.test(path) && body?.text) {
    window._demoMessages = window._demoMessages || [];
    const ts = new Date().toISOString().slice(0,19).replace('T',' ');
    window._demoMessages.push({
      id:'demo_msg_'+Date.now(), account_id:'demo_acc1',
      user_id: currentUser?.id || 'u_master', user_name: currentUser?.name || '데모 관리자',
      role: currentUser?.role || 'master', text: body.text, created_at: ts,
    });
    return {ok:true};
  }
  return {ok:true};
...
function _buildDemoResponse(path) {
  ...
  if (/\/audit/.test(path))            return {audit: _genDemoAuditRows()};
  if (/\/messages/.test(path))         return {messages: (window._demoMessages||[]).slice().reverse()};
...
function _initDemoMode() {
  ...
  // API를 데모 응답으로 완전 대체 — 실제 네트워크 요청은 전혀 발생하지 않는다
  DEEPFLE_API.USE_MOCK = false;
  DEEPFLE_API.live     = true;
  const _demoGet = async (path) => _buildDemoResponse(path);
  DEEPFLE_API.get     = _demoGet;
  DEEPFLE_API.post    = async (path, body) => _buildDemoPostResponse(path, body);
  DEEPFLE_API.patch   = async () => ({ok:true});
  DEEPFLE_API.put     = async () => ({ok:true});
  DEEPFLE_API.del     = async () => ({ok:true});
  DEEPFLE_API.request = async (method, path, body) =>
    method === 'GET' ? _buildDemoResponse(path) : _buildDemoPostResponse(path, body);
...
(async function bootstrap(){
  ...
  // ?share=TOKEN 자동 감지 — 로그인 없이 공유 뷰로 진입 (리포트 이메일 발송에 포함되는 링크)
