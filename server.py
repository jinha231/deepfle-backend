...
class Api:
    ...
    def raw_hierarchy(body, user, conn, params):
        ...
        # 수기 전환 데이터 입력 값을 "(수기입력)" 합성 행으로 병합
        mc_sql = "SELECT date,media,conv_id,value FROM manual_conv_data WHERE account_id=?"
        mc_args = [aid]
        if from_date: mc_sql += " AND date>=?"; mc_args.append(from_date)
        if to_date:   mc_sql += " AND date<=?"; mc_args.append(to_date)
        if media_key: mc_sql += " AND media=?"; mc_args.append(media_key)
        mc_rows = conn.execute(mc_sql, mc_args).fetchall()
        if mc_rows:
            conv_labels = {r["id"]: r["solution_metric"] for r in
                           conn.execute("SELECT id,solution_metric FROM conversion_settings WHERE account_id=?", (aid,)).fetchall()}
            for r in mc_rows:
                label = conv_labels.get(r["conv_id"], f"전환#{r['conv_id']}")
                rows.append({
                    "date": r["date"], "media_key": r["media"],
                    "campaign": f"(수기입력·{label})", "campaign_type": "", "adgroup": "", "keyword": "", "creative": "",
                    "cost": 0, "imp": 0, "click": 0,
                    "conv": r["value"], "conv_native": 0, "conv_ga4": 0, "conv_mmp": 0, "conv_manual": r["value"],
                })
