...
class Api:
    ...
    def list_metric_data(body, user, conn, params):
        ...
        # Raw 업로드(수동 업로드/수기 매체 업로드)로 입력된 manual_metrics도 병합 — 대시보드·미디어리포트가
        # Raw 업로드/다운로드와 같은 데이터를 보도록 한다. 캠페인 등 계층 없이 날짜·매체 단위로 합산.
        mm_sql = ("SELECT date, media, SUM(cost) as cost, SUM(imp) as imp, SUM(click) as click, "
                  "SUM(conv_native+conv_ga4+conv_mmp+conv_manual) as conv "
                  "FROM manual_metrics WHERE account_id=? AND date>=? AND date<=?")
        mm_args = [aid, d_from, d_to]
        if media: mm_sql += " AND media=?"; mm_args.append(media)
        mm_sql += " GROUP BY date, media"
        for r in conn.execute(mm_sql, mm_args).fetchall():
            for mk in ("cost", "imp", "click", "conv"):
                v = r[mk]
                if v:
                    data.append({"account_id": aid, "date": r["date"], "media": r["media"],
