...
def _send_emailjs(to_email: str, to_name: str, code_or_url: str, expires_in: str,
    ...
    req = _ur.Request(
        ...
        data=payload,
        headers={"Content-Type": "application/json", "origin": "https://jinha231.github.io"},
        method="POST",
    )
    try:
...
class Api:
    ...
    def create_conversion(body, user, conn, params):
        ...
        cur = conn.execute(
            "INSERT INTO conversion_settings (account_id,source,source_metric,solution_metric,value_type,config) VALUES (?,?,?,?,?,?)",
    ...
    def update_conversion(body, user, conn, params):
        ...
        if not role or not can_edit(role):
            return 403, {"error": "편집 권한이 필요합니다."}
        b = body or {}
        for f in ("source", "source_metric", "solution_metric", "value_type", "active"):
            if f in b:
    ...
    # ───────── 설정: 매체연동 매핑 (제공지표 → 솔루션 표준명) ─────────
    @staticmethod
    def list_media_map(body, user, conn, params):
        aid = params["account_id"]
        role = account_role(conn, user["sub"], aid)
        if not role:
            return 403, {"error": "접근 권한이 없습니다."}
        rows = conn.execute("SELECT * FROM media_metric_map WHERE account_id=? ORDER BY media,id", (aid,)).fetchall()
        return 200, {"mappings": [dict(r) for r in rows], "canEdit": can_edit(role)}

    @staticmethod
    def upsert_media_map(body, user, conn, params):
        aid = params["account_id"]
        role = account_role(conn, user["sub"], aid)
        if not role or not can_edit(role):
            return 403, {"error": "편집 권한이 필요합니다."}
        b = body or {}
        media, mk = b.get("media"), b.get("metric_key")
        if not media or not mk:
            return 400, {"error": "media, metric_key는 필수입니다."}
        existing = conn.execute(
            "SELECT id FROM media_metric_map WHERE account_id=? AND media=? AND metric_key=?",
            (aid, media, mk)).fetchone()
        if existing:
            conn.execute("UPDATE media_metric_map SET provider_field=?,solution_name=?,active=? WHERE id=?",
                         (b.get("provider_field", ""), b.get("solution_name", mk),
                          1 if b.get("active", True) else 0, existing["id"]))
            mid = existing["id"]
        else:
            cur = conn.execute(
                "INSERT INTO media_metric_map (account_id,media,metric_key,provider_field,solution_name) VALUES (?,?,?,?,?)",
                (aid, media, mk, b.get("provider_field", ""), b.get("solution_name", mk)))
            mid = cur.lastrowid
        conn.commit()
        return 200, {"ok": True, "id": mid}

    @staticmethod
    def delete_media_map(body, user, conn, params):
        mid = params["map_id"]
        row = conn.execute("SELECT * FROM media_metric_map WHERE id=?", (mid,)).fetchone()
        if not row:
            return 404, {"error": "매핑을 찾을 수 없습니다."}
        role = account_role(conn, user["sub"], row["account_id"])
        if not role or not can_edit(role):
            return 403, {"error": "편집 권한이 필요합니다."}
        conn.execute("DELETE FROM media_metric_map WHERE id=?", (mid,))
        conn.commit()
        return 200, {"ok": True}
    ...
    def metric_catalog(body, user, conn, params):
        ...
        if not role:
            return 403, {"error": "접근 권한이 없습니다."}
        base = conn.execute(
            "SELECT metric_key,solution_name FROM media_metric_map WHERE account_id=? AND active=1 GROUP BY metric_key ORDER BY id",
            (aid,)).fetchall()
        conv = conn.execute(
            "SELECT solution_metric,source,value_type FROM conversion_settings WHERE account_id=? AND active=1 ORDER BY id",
            (aid,)).fetchall()
        return 200, {
            "base": [{"key": r["metric_key"], "name": r["solution_name"]} for r in base],
    ...
    def list_metric_data(body, user, conn, params):
        ...
        return 200, {"data": [dict(r) for r in rows], "count": len(rows)}
    ...
    def pull_metric_data(body, user, conn, params):
        ...
        media_list = conn.execute(
            "SELECT DISTINCT media FROM media_metric_map WHERE account_id=? AND active=1", (aid,)
        ).fetchall()
        if not media_list:
...
ROUTES = [
    ...
    ("GET",  r"^/api/accounts/(?P<account_id>[^/]+)/media-metric-map$", Api.list_media_map, True),
    ("POST", r"^/api/accounts/(?P<account_id>[^/]+)/media-metric-map$", Api.upsert_media_map, True),
    ("DELETE", r"^/api/media-metric-map/(?P<map_id>\d+)$",     Api.delete_media_map, True),
